...
class SplineTrack:
    ...
    def __init__(
        ...
        if path_to_file is not None:
            ...
            raise NotImplementedError(
                "neither coordinates or the path to the coordinates was given"
            )
        ...
        for i in range(len(params) - 1):
            ...
            coords[0, i, :] = coords[0, i, :] + safety_margin * direction / np.linalg.norm(
                direction + 0.001
            )

            # shrink out line
            direction = coords[1, i, :] - coords[2, i, :]
            coords[2, i, :] = coords[2, i, :] + safety_margin * direction / np.linalg.norm(
                direction + 0.001
            )

        # 3 set track #
        ###############
        self.midline = EnhancedSpline(coords[1, :, :], params, self.track_length)
        ...
        if path_to_traj is not None:
            ...
            self.delta_traj = self.trajectory.find_theta_slow(self.midline.get_coordinate(0))
        else:
    ...
    def get_derivative(self, theta, line="mid") -> np.array:
        """
